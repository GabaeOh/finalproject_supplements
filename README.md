...
- 코로나 19 이후 관심도 증가  
- 시장전망 증가 예측
- 정보 부족 해소 하기 위해 설문조사 기반으로 개인정보 맞춤형
  - 출처 : [코로나19시대 국민 체중 관리 현황 및 비만 인식 조사](http://general.kosso.or.kr/html/?pmode=BBBS0001300004&smode=view&seq=1372)
...
| 순번 | 분석 | 도출된 결과 | 개선 사항 |
| --- | --- | --- | --- |
| 1 | 인사이트 도출 결과 | 구매자가 효능을 가장 중요시 함 | 효능을 강조하기 위하여 효능 뱃지 색깔 변경, 효능에 따른 랭킹을 구현하여 보조제 추천 |
| 2 | 공공데이터 분석 | 남녀의 신장과 체중을 가지고와 BMI 사이트에 통계 구현 | 쿼리문을 작성하여 bar/Pie 차트를 구현하여 시각화 |
| 3 | 커뮤니티 게시글 분석 | 월 별 게시글 작성자 수를 확인하여 사이트 이용자수 확인 | 관리자페이지에 커뮤니티 월별게시글 수를 line chart로 구현하여 시각화 |
| 4 | 설문조사 결과 분석 | 의뢰팀의 요구사항 | 보조제별, 복용 원인별 각각의 통계를 시각화하여 어떤 제품이 가장 많이 섭취되었음을 보여줌 |
